...
def rearrange(model1, model2, comb, cat):
    ...
    with open(model1 + 'R', 'w') as f:
        f.write(' '.join([str(i) for i in list(rm1.shape)]) + '\n')
    f.close()

    with open(model2 + 'R', 'w') as f:
        f.write(' '.join([str(i) for i in list(rm2.shape)]) + '\n')
        rm2.to_csv(f, sep = ' ', header=None)
    f.close()

    with open(comb + 'R', 'w') as f:
        f.write(' '.join([str(i) for i in list(RproTran.shape)]) + '\n')
        RproTran.to_csv(f, sep = ' ', header=None)
    f.close()
...
#rearrange('../nmdsExp_mtv2/80_20/trainv_2_80_1_trained', '../nmdsExp_mtv2/80_20/trainv_2_20_2_trained', '../nmdsExp_mtv2/80_20/org_comb', 'd s c l p')
